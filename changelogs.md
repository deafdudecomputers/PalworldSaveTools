...
- **The app remembers your save folder** — your last used save folder is kept and offered again next time, so you don't have to dig for Level.sav by hand. Opening any tool from the Tools tab also used to quietly drop the save you had loaded, so the next thing you did asked for the folder again; the save is now reloaded when you close the tool. The rest of the pickers that used to open somewhere random — Global Pal Storage, WorldOption, Slot Injector, Character Transfer, Fix Host Save and the GamePass Steam folder prompt — all start in your last used save folder. GamePass sessions are never remembered, since those live in a local folder that is removed each time.
