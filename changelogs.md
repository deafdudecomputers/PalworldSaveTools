#2.2.9
- **Base import no longer corrupts structure concrete models** - structures (walls, roofs, fences, etc.) whose concrete model data is empty were having fake 32-byte ID blobs injected into them on import, which made the game treat every imported structure as unsupported ("not enough support", "missing foundations") so nothing new could be built on the imported base. Empty concrete data is now left untouched; only objects that genuinely carry raw concrete bytes get their IDs rewritten.
- **Base import now rewires structure connector links to the imported copy** - structure-to-structure connection references (what keeps a chest attached to its foundation, etc.) were left pointing at the source save's object IDs after an import or clone. Cross-world imports produced dangling references, and same-world imports wired the imported copy into the original base's connector network. Links are now remapped to the imported copy's fresh IDs.
