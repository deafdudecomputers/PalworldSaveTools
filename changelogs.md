#2.2.5
- **macOS Gatekeeper delay fixed** — ad-hoc code signing was causing a ~2min gatekeeper prompt delay on first launch (macOS tries OCSP/notarization checks that time out). Removed ad-hoc signing from release builds — app ships unsigned, same as pre-2.2.x behavior. Gatekeeper prompt now appears instantly.
