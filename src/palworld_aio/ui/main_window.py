...
class MainWindow(QMainWindow):
    ...
    def _repair_items(self):
        ...
        run_with_loading(on_finished, task)
    def _fix_broken_connectors(self):
        if not constants.loaded_level_json:
            self._show_warning(t('Error'), t('error.no_save_loaded'))
            return
        def task():
            return fix_broken_connectors(self)
        def on_finished(result):
            self.refresh_all()
            self._show_info(t('Done'), t('deletion.connectors_fixed', fixed=result['fixed'], remaining=result['remaining']))
        run_with_loading(on_finished, task)
